...
Known scope limitation: spread is not tracked here - that requires order
book (bid/ask) data at fill time, which isn't part of this phase (see
docs/PHASE_0_ARCHITECTURE_RESEARCH.md section 7's "later" list:
microstructure/order-book data). Revisit once that data is available.
"""
...
def compare_fill(intent: OrderIntent, fill: Fill) -> FillComparison:
    ...
    elif abs(fill.filled_quantity - intent.quantity) > 1e-9:
