...
@dataclass(frozen=True, slots=True)
class BinanceRawCollectorConfig:
    market_type: str
    symbols: tuple[str, ...]
    flush_interval_secs: float
    max_batch_events: int
...
def load_binance_raw_collector_config(
    path: Path = DEFAULT_RAW_COLLECTOR_CONFIG,
) -> BinanceRawCollectorConfig:
    value = yaml.safe_load(Path(path).read_text(encoding="utf-8"))
    if not isinstance(value, dict) or value.get("schema_version") != 1:
        raise ValueError("raw collector config must use schema_version 1")
    binance = value.get("binance")
    if not isinstance(binance, dict):
        raise ValueError("raw collector config is missing the binance section")
    config = _build_binance_config(binance)
    if config.symbols != INITIAL_V2_BINANCE_SYMBOLS:
        raise ValueError(
            f"initial v2 Binance raw universe must be exactly "
            f"{INITIAL_V2_BINANCE_SYMBOLS}; expansion requires a reviewed "
            "master-plan change"
        )
    return config


def _build_binance_config(value: dict[str, Any]) -> BinanceRawCollectorConfig:
    try:
        config = BinanceRawCollectorConfig(
            market_type=str(value["market_type"]),
            symbols=tuple(str(symbol) for symbol in value["symbols"]),
            flush_interval_secs=float(value["flush_interval_secs"]),
            max_batch_events=int(value["max_batch_events"]),
