"""CLI: run the live OKX long/short account-ratio poller continuously,
meant for `docker compose run -d` like scripts/collect_okx_open_interest.py.
...
Usage:
    python scripts/collect_okx_long_short_ratio.py --inst-id BTC-USDT-SWAP
"""
...
from src.data.okx_derivatives_client import VALID_PERIODS
from src.data.okx_derivatives_collector import OkxLongShortRatioCollector
...
def collect(
    ...
    period: str = typer.Option(
        "5m", help=f"Ratio aggregation window, one of {VALID_PERIODS}."
    ),
    ...
    log.info(
        "starting okx long/short ratio collector",
        inst_id=inst_id,
        period=period,
