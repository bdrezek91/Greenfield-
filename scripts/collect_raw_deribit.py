"""Run the lossless Greenfield v2 Deribit collector for BTC and ETH
perpetuals.

Structurally mirrors scripts/collect_raw_okx.py - same start-gate binding
(src.data.raw_collector_start_gate.validate_raw_collector_start), same
config-driven instrument/timing setup. Requires its own soak marker
...
def collect(
    ...
    instrument: Annotated[
        str | None, typer.Option(help="Run one configured instrument in this process.")
    ] = None,
    ...
    config = load_deribit_raw_collector_config(config_path)
    if instrument is not None and instrument not in config.instruments:
        raise typer.BadParameter(
            f"instrument must be one of {config.instruments}", param_hint="--instrument"
        )
    instruments = (instrument,) if instrument is not None else config.instruments
    ...
    log.info(
        "starting lossless Deribit raw collector",
        ...
        subscribe_channels=collector.subscribe_channels,
    )
