"""scripts/collect_raw_coinbase.py: config-driven startup and fail-closed
start-gate wiring, mirroring scripts/collect_raw_okx.py's behavior.

Only exercises paths that return before RawCoinbaseCollector.run_forever()
...
from scripts.collect_raw_coinbase import app

runner = CliRunner()


def test_invalid_product_id_is_rejected_before_any_connection(
    ...
    result = runner.invoke(
        app, ["--data-dir", str(tmp_path), "--product-id", "NOT-A-REAL-PRODUCT"]
    )

    assert result.exit_code != 0
    assert "product_id must be one of" in str(result.output)
