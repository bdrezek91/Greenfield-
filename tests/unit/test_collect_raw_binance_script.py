"""scripts/collect_raw_binance.py: config-driven startup and fail-closed
start-gate wiring, mirroring scripts/collect_raw_okx.py's behavior.

Only exercises paths that return before RawBinanceCollector.run_forever()
...
from scripts.collect_raw_binance import app

runner = CliRunner()


def test_invalid_symbol_is_rejected_before_any_connection(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("GREENFIELD_SOAK_ID", raising=False)
    monkeypatch.delenv("GREENFIELD_DEPLOY_COMMIT", raising=False)

    result = runner.invoke(app, ["--data-dir", str(tmp_path), "--symbol", "NOTASYMBOL"])
