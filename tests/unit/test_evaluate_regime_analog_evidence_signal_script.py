"""scripts/evaluate_regime_analog_evidence_signal.py: symbol/stride
...
from scripts.evaluate_regime_analog_evidence_signal import app
...
def test_non_positive_stride_is_rejected(tmp_path: Path) -> None:
    result = runner.invoke(
        app,
        [
            "--data-dir",
    ...
    assert result.exit_code != 0
    assert "positive" in str(result.output).lower()
