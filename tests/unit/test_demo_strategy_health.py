...
from src.execution.demo_strategy_health import DemoStrategyHealthPublisher


def test_health_publishes_atomic_json_and_prometheus(tmp_path: Path) -> None:
    path = tmp_path / "health.json"
    DemoStrategyHealthPublisher(path).publish(
        {
    ...
    assert 'greenfield_demo_strategy_status{status="OPEN"} 1' in metrics
    assert "greenfield_demo_strategy_operator_forced 1" in metrics
    assert "greenfield_demo_strategy_active_trade 1" in metrics
